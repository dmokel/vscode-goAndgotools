...
```
此扩展是高度可配置的，因此提供了许多设置。可以通过修改您的[用户或工作区设置](https://code.visualstudio.com/docs/getstarted/settings)来配置这些设置。
```
...
**NOTE: Many of these settings don't apply if you are using [`gopls`](https://github.com/golang/vscode-go/blob/master/docs/gopls.md). Learn more about [`gopls`-specific settings](https://github.com/golang/vscode-go/blob/master/docs/gopls.md#ignored-settings).**
...
Alternate tools or alternate paths for the same tools used by the Go extension. Provide either absolute path or the name of the binary in GOPATH/bin, GOROOT/bin or PATH. Useful when you want to use wrapper script for the Go tools or versioned tools from [https://gopkg.in](https://gopkg.in/).
