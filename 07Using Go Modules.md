...
```
Go 1.11和1.12包含了初步的[对Module的支持](https://golang.org/doc/go1.11#modules)， Go的[新的依赖管理系统](https://blog.golang.org/versioning-proposal)，这使得依赖版本信息显式且更容易管理。这篇博客文章介绍了开始使用Modules所需要的基本操作。
```
...
```
Module是一个[Go包](https://golang.org/ref/spec#包)的集合，它存储在带有'Go.mod'文件的文件树根目录中。'go.mod'文件定义了Module的*模块路径*，这也是根目录使用的导入路径，以及它的*依赖性需求*，这是成功构建所需的其他模块。每个依赖项需求都被编写为模块路径和特定的[语义版本](http://semver.org/)。
```
...
```
在Go 1.11中，当当前目录或任何父目录有一个' Go.mod ' '时，Go命令允许使用Modules，如果目录是在$GOPATH/src'外时。(在' $GOPATH/src '中，为了兼容性，go命令仍然在旧的GOPATH模式下运行，即使是' go.mod '被发现。有关详细信息，请参阅[go命令文档](https://golang.org/cmd/go/#hdr-初起_module_support)。)从Go 1.13开始，Modules模式将是所有开发的默认模式。
```

This post walks through a sequence of common operations that arise when developing Go code with modules:

```
这篇文章介绍了在开发带有Modules的Go代码时出现的一系列常见操作:
```
...
```
此时，目录包含一个包，而不是一个module，因为没有' go.mod '文件。如果我们在' /home/gopher/hello '中工作，现在运行' go test '，我们会看到:
```
...
```
最后一行总结了整个包测试。因为我们工作在' $GOPATH '之外，也在任何模块之外，' go '命令不知道当前目录的导入路径，并根据目录名' _/home/gopher/hello '创建一个假的路径。
```
...
- [Keeping Your Modules Compatible](https://blog.golang.org/module-compatibility)
...
- [The App Engine SDK and workspaces (GOPATH)](https://blog.golang.org/appengine-gopath)
